...
# Define classes
class House:
    def __init__(self, coordinates, height, elevation, type):
    ...
    def draw(self):
        if self.type == 'MultiPolygon':  # Check if it's a multipolygon
            for polygon in self.coordinates:
                if len(polygon) >= 3:
                    py.draw.polygon(screen, house_color, polygon)
        else:  # It's a polygon or courtyard
            if len(self.coordinates[0]) >= 3:
                py.draw.polygon(screen, house_color, self.coordinates[0])
...
class Shadow:
    ...
    def draw(self):
        if self.type == 'MultiPolygon':  # Check if it's a multipolygon
            ...
            if len(self.coordinates[0]) >= 3:
                py.draw.polygon(screen, shadow_color, self.coordinates[0])

# Create map objects
map = [
    House(row['coordinates'], row['height'], row['elevation'], row['type']) for _, row in df.iterrows()
]
...
def scale_positions(map, shade_map, lon_min, lon_max, lat_min, lat_max):
...
scale_positions(map, shade_map, lon_min, lon_max, lat_min, lat_max)
